   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_747)
            {
               Core.crash("Error catched in EventDispatcher; some event listener crashed!",var_1154.errorID,var_1154);
            }
            return _result == const_484;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_249[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1045) : uint(const_484);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_249)
            {
               _loc2_ = var_249[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_249[_loc1_];
            }
            var_249 = null;
