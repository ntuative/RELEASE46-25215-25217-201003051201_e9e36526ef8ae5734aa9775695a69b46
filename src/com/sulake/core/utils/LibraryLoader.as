package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1390:int = 5;
      ...
      protected var var_433:uint = 0;
      
      protected var var_14:Loader;
      
      protected var var_2260:String = "";
      
      protected var var_317:URLRequest;
      ...
      protected function prepareLibrary() : void
      {
         ...
         if(var_358 == null)
         {
            failure("Failed to find resource class \"" + _name + "\" in library " + var_317.url + "!");
            return;
         }
         try
         {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_433 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_14.contentLoaderInfo.url + "\"");
               var_433 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_363 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_363 + " for file \"" + var_14.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
         ...
         if(var_433 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_433 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_433 == LibraryLoader.STATE_READY)
         {
            var_1362 = true;
            prepareLibrary();
            removeEventListeners();
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_926 > 0)
            {
               var_14.close();
               var_14.unload();
               addRequestCounterToUrlRequest(var_317,const_1390 - var_926);
               var_14.load(var_317,_context);
               --var_926;
               return true;
            }
