package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1173:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String)
      {
         ...
         _assets = param1;
         var_1173 = param2;
         var_1172 = "http://" + param3 + "/client";
         var_1173.init();
         var_1173.addEventListener(HabboWeb.const_200,requestClientURL);
      }
      ...
      {
         var _loc2_:String = var_1172;
         if(_assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + var_1172);
         }
         var _loc3_:URLRequest = new URLRequest(var_1172);
         if(_assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            var_1173.requestReAuthenticate();
         }
         else
         {
            try
            {
               var_1431 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               var_1793 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               if(false)
               {
