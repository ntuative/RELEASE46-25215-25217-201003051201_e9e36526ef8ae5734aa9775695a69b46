package com.sulake.habbo.navigator.inroom
{
   ...
   public class RoomEventViewCtrl
   {
      ...
      private var var_2092:IButtonWindow;
      ...
      private function onEditButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc5_:String = var_713.getText();
      ...
      {
         var_864.visible = false;
         var_1525.visible = true;
         var_2093.text = _navigator.getText("roomevent_type_" + param1.eventType);
         _eventNameInput.setText(param1.eventName);
         var_713.setText(param1.eventDescription);
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         if(_window == null)
         {
            ...
            var_1525 = IWindowContainer(find("event_type_disabled_cont"));
            var_2093 = ITextWindow(find("event_type_disabled_text"));
            var_1523 = ITextWindow(find("create_caption"));
            var_1524 = ITextWindow(find("edit_caption"));
            var_1292 = IButtonWindow(find("create_button"));
            var_1290 = IButtonWindow(find("edit_button"));
            var_2092 = IButtonWindow(find("cancel_button"));
            var_1291 = IButtonWindow(find("end_button"));
            var_864 = IDropMenuWindow(find("event_type"));
            prepareEventTypes();
            Util.setProcDirectly(var_1292,onCreateButtonClick);
            Util.setProcDirectly(var_1290,onEditButtonClick);
            Util.setProcDirectly(var_1291,onEndButtonClick);
            Util.setProcDirectly(var_2092,onCancelButtonClick);
            _eventNameInput = new TextFieldManager(_navigator,getInput("event_name"),25);
            var_713 = new TextFieldManager(_navigator,getInput("event_desc"),100);
      ...
      private function isMandatoryFieldsFilled() : Boolean
      {
         ...
         if(!var_713.checkMandatory(_navigator.getText("navigator.eventsettings.descerr")))
         {
            return false;
         }
         return true;
