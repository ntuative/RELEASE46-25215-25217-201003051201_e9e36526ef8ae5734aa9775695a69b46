package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_2225:MarketplaceItemStats;
      ...
      {
         if(!var_159 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_159.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1441,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_1600 || param1.furniTypeId != var_1596)
         {
            return;
         }
         var_2225 = param1;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_159.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_159.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_159.add(parser.offerId,updateItem);
            }
            var_159.remove(parser.requestedOfferId);
            showConfirmation(const_1453,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_2070 = _loc3_.creditsWaiting;
         var_206 = new Map();
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_159.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2224,var_2223,var_2130,var_2226);
      }
