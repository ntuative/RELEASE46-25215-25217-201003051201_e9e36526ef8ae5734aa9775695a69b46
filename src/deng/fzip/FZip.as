package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_569))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_569,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_91 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_569,"EOF"));
            }
         }
      }
      ...
      private function parseLocalfile(param1:IDataInput) : Boolean
      {
         if(var_469.parse(param1))
         {
            ...
            if(false)
            {
               var_123["null"] = var_469;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1315,var_469));
            var_469 = null;
      ...
      {
         var _loc2_:* = null;
         if(var_45 != null && var_123 != null && param1 < var_45.length)
         {
